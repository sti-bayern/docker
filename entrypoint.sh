...
if [ -z "$(ls -A /var/lib/postgresql)" ]; then
    ...
    su postgres -c "/usr/lib/postgresql/$POSTGRES/bin/postgres --single --config-file=/etc/postgresql/$POSTGRES/main/postgresql.conf" <<< "GRANT ALL PRIVILEGES ON app TO app;"
fi
